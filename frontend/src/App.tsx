...
import { Bell, BriefcaseBusiness, Check, CheckCircle2, ChevronRight, CircleHelp, FileText, FolderOpen, Home, LockKeyhole, LogOut, MessageSquare, Upload, X } from 'lucide-react'
...
export function App() {
  ...
  return <div className="app-shell">
    ...
    <aside>
      {role === 'cpa' ? <nav aria-label="CPA navigation"><a className={route === '#cpa-home' || route === '#cpa-clients' ? 'active' : ''} href="#cpa-home"><Home size={19}/>Dashboard</a><a className={route === '#cpa-status' ? 'active' : ''} href="#cpa-status" onClick={()=>{if(!selectedClient)setSelectedClient({id:'flores-2025',name:'Maya & Daniel Flores'})}}><CheckCircle2 size={19}/>Return status</a><a className={route === '#cpa-review' ? 'active' : ''} href={selectedClient?'#cpa-review':'#cpa-home'}><FileText size={19}/>Review queue<span className="nav-count">4</span></a><a className={route === '#cpa-messages' ? 'active' : ''} href="#cpa-messages"><MessageSquare size={19}/>Collaboration<span className="nav-count">3</span></a></nav> : <nav aria-label="Client navigation"><a className={route === '#home' || route === '' ? 'active' : ''} href="#home"><Home size={19}/>Home</a><a className={route === '#status' ? 'active' : ''} href="#status"><CheckCircle2 size={19}/>Return status</a><a className={route === '#documents' ? 'active' : ''} href="#documents"><FolderOpen size={19}/>Documents<span className="nav-count">2</span></a><a className={route === '#messages' ? 'active' : ''} href="#messages"><MessageSquare size={19}/>Collaboration<span className="nav-count">2</span></a></nav>}
    ...
    <main>{role === 'cpa' ? (route === '#cpa-messages' ? <Collaboration role="cpa"/> : route === '#cpa-review' && selectedClient ? <ChallengeEight role="cpa" clientName={selectedClient.name}/> : route === '#cpa-status' && selectedClient ? <ReturnStatus role="cpa" clientName={selectedClient.name} onReview={()=>{location.hash='#cpa-review'}}/> : <CpaDashboard onReview={client => { setSelectedClient(client); location.hash = '#cpa-status' }}/>) : route === '#status' ? <ReturnStatus role="client"/> : route === '#documents' ? <ChallengeEight role="client" onUploadComplete={async()=>{const task=workspace.tasks.find(item=>item.status==='ready'&&item.type==='document');if(task)setWorkspace(await completeTask(task.id));location.hash='#home'}}/> : route === '#messages' ? <Collaboration role="client"/> : <>
