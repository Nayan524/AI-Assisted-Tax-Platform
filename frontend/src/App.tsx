...
export function App() {
  ...
  return <div className="app-shell">
    ...
    <aside>
      {role === 'cpa' ? <nav aria-label="CPA navigation"><a className={routePath === '#cpa-home' || routePath === '#cpa-clients' ? 'active' : ''} href="#cpa-home"><Home size={19}/>Dashboard</a><a className={`${routePath === '#cpa-status' ? 'active' : ''} ${!selectedClient?'disabled':''}`} href={selectedClient?'#cpa-status':'#cpa-home'} aria-disabled={!selectedClient}><CheckCircle2 size={19}/>Return status</a><a className={`${routePath === '#cpa-questionnaire' ? 'active' : ''} ${!selectedClient?'disabled':''}`} href={selectedClient?'#cpa-questionnaire':'#cpa-home'} aria-disabled={!selectedClient}><CircleHelp size={19}/>Questionnaire</a><a className={`${routePath === '#cpa-review' ? 'active' : ''} ${!selectedClient?'disabled':''}`} href={selectedClient?'#cpa-review':'#cpa-home'} aria-disabled={!selectedClient}><FileText size={19}/>Review queue</a><a className={`${routePath === '#cpa-messages' ? 'active' : ''} ${!selectedClient?'disabled':''}`} href={selectedClient?'#cpa-messages':'#cpa-home'} aria-disabled={!selectedClient}><MessageSquare size={19}/>Messages</a></nav> : <nav aria-label="Client navigation"><a className={routePath === '#home' || routePath === '' ? 'active' : ''} href="#home"><Home size={19}/>Home</a><a className={routePath === '#status' ? 'active' : ''} href="#status"><CheckCircle2 size={19}/>Return status</a><a className={routePath === '#documents' ? 'active' : ''} href="#documents"><FolderOpen size={19}/>Documents</a><a className={routePath === '#messages' ? 'active' : ''} href="#messages"><MessageSquare size={19}/>Messages</a></nav>}
