...
function ClientDocuments({onContinue}:{onContinue?:()=>void}) {
  ...
  return <div className="review-page client-documents"><button className="status-back-home" onClick={()=>{location.hash='#home'}}><ArrowLeft size={15}/>Back to Home</button>
    ...
    <div className="screen-tabs client-document-tabs" role="tablist"><button className={clientTab==='upload'?'selected':''} onClick={()=>setClientTab('upload')}><Upload size={17}/>Upload documents</button><button className={clientTab==='files'?'selected':''} onClick={()=>setClientTab('files')}><FileText size={17}/>Uploaded files <span className="review-count">{3+uploaded.length-(removed.some(item=>item.name==='2025 W-2 · Northstar Design Group')?1:0)}</span></button></div>
    {clientTab==='upload'?<>
      ...
      {uploaded.length>0&&<section className="recent-uploads">{uploaded.map((name,index)=><div className="uploaded-file-row" key={`${name}-${index}`}><span className="file-type">{name.split('.').pop()?.toUpperCase()||'FILE'}</span><div><strong>{name}</strong><small>Uploaded this session · Received</small></div><span className="client-status received"><Check size={12}/>Received</span><button className="remove-file-button" onClick={()=>setRemovalTarget({name,reviewed:false})} aria-label={`Remove ${name}`}><Trash2 size={14}/>Remove</button></div>)}</section>}
      <section className="document-grid">{!removed.some(item=>item.name==='2025 W-2 · Northstar Design Group')&&<ClientDocument title="2025 W-2" subtitle="Northstar Design Group · Daniel Flores" status="Under CPA review" meta="Uploaded Aug 18" onRemove={()=>setRemovalTarget({name:'2025 W-2 · Northstar Design Group',reviewed:true})}/>}<ClientDocument title="2025 W-2" subtitle="Beacon Health · Maya Flores" status="Received" meta="Uploaded Aug 12" complete/><ClientDocument title="Engagement letter" subtitle="MiraFlores Tax · Signed copy" status="Complete" meta="Signed Aug 10" complete/></section>
