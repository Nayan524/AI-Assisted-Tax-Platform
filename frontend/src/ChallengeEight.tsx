...
import { additionalDocuments } from './mockDocumentLibrary'
...
function ClientDocuments({onContinue}:{onContinue?:()=>void}) {
  ...
  return <div className="review-page client-documents"><button className="status-back-home" onClick={()=>{location.hash='#home'}}><ArrowLeft size={15}/>Back to Home</button>
    ...
    <div className="screen-tabs client-document-tabs" role="tablist"><button className={clientTab==='upload'?'selected':''} onClick={()=>setClientTab('upload')}><Upload size={17}/>Upload documents</button><button className={clientTab==='files'?'selected':''} onClick={()=>setClientTab('files')}><FileText size={17}/>Uploaded files <span className="review-count">{6+additionalDocuments.length+uploaded.length-(removed.some(item=>item.name==='2025 W-2 · Northstar Design Group')?1:0)}</span></button></div>
    {clientTab==='upload'?<>
      ...
      <section className="uploaded-document-library"><div className="uploaded-library-heading"><div><h2>Additional tax documents</h2><p>50 synthetic records for testing document volume and navigation.</p></div><span>{additionalDocuments.length} files</span></div>{additionalDocuments.map(document=><div className="uploaded-library-row" key={document.id}><span className="file-type">{document.form}</span><div><strong>{document.title}</strong><small>{document.subtitle} · {document.meta}</small></div><span className="client-status received"><Check size={12}/>Received</span><button className="outline-button" onClick={()=>setPreview({name:`${document.title} · ${document.subtitle}.pdf`,url:document.url,type:'pdf'})}>View file</button></div>)}</section>
...
function Documents({onReview}: {onReview: () => void}) {
  const documents = [
    ...
    ...additionalDocuments.map(document=>({name:`${document.title} — ${document.subtitle}.pdf`,url:document.url})),
  ]
