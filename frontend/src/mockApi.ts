...
const workspace: ClientWorkspace = {
  ...
  tasks: [
    ...
    { id: 'w2', title: 'Upload Daniel’s W-2', description: 'From Northstar Design Group', type: 'document', status: 'ready', estimate: '1 document', dueLabel: 'Needed next' },
    { id: 'interest', title: 'Upload interest statements', description: 'Forms 1099-INT from your bank accounts', type: 'document', status: 'waiting', dueLabel: 'After questionnaire' },
    { id: 'engagement', title: 'Review engagement letter', description: 'Signed by Maya Flores', type: 'review', status: 'complete' },
  ],
}
...
export async function completeTask(id: string) {
  ...
  if (id === 'profile') {
    const next = workspace.tasks.find(item => item.id === 'interest')
